...
class Event(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column("event_id", Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4)
    source: Mapped[str] = mapped_column("source_type", String(128), nullable=False)
    ...
    occurred_at: Mapped[datetime] = mapped_column("timestamp", DateTime(timezone=True), nullable=False, server_default=func.now())
